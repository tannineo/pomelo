...
Application.afterStart = function(cb) {
  ...
  var self = this;
  appUtil.optComponents(this.loaded, Constants.RESERVED.AFTER_START, function(
    err
  ) {
...
Application.configure = function(env, type, fn) {
  ...
  if (env === Constants.RESERVED.ALL || contains(this.settings.env, env)) {
    if (
      ...
      contains(this.settings.serverType, type)
    ) {
      fn.call(this);
    }
  }
...
/**
 * 从servers.json里根据serverId获取serverInfo
